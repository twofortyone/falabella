...
dff3p = bd[bd['estatus final'] == 'Con F3 Proveedor ']
nf3p = dff3p.shape[0]
...
# Análisis de F4 
dff4 = bd[(bd['estatus final']=='Dado de baja por entregado a cliente -(Autoriza Gerencia de Control Operacional)') 
        ...
        | (bd['estatus final']=='Cierre x F4 Cobrado a terceros')]
...
# Análisis de KPI
dfkpi = bd[(bd['estatus final']=='Recibido en CD')]
nkpi = dfkpi.shape[0]
...
bdtotal = bd.merge(f3, how='left', left_on=['f3','upc'], right_on=['nro devolucion','upc']).merge(f4, how='left', left_on=['f4','upc'], right_on=['nro. red. inventario','upc']).merge(kpi, how='left', left_on='nro_f12', right_on='entrada')
