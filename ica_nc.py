...
class InternalControlAnalysis:
    ...
    def f4_verify(self, f4, status, yyyy):
        ...
        if df2.empty == False:
            ...
            ne = self.get_notfound( df3, f4, [self.fcols[1], self.upc_column], ['nro_red_inventario','upc'], 'nro_red_inventario', 'F4|UPC|QTY')
            df4 = pd.merge(df3, f4, left_on=[self.fcols[1], self.upc_column], right_on=['nro_red_inventario','upc'])
            if df4.empty ==False: 
                ...
                if df6.empty == False:
                    df7 = self.get_diffqty_pro(df6, self.qty_column, 'cantidad',self.fcols[5],'nro_red_inventario', comment)
    ...
    def f5_verify(self, f5, status, yyyy, dqpcol):
        ...
        if df2.empty ==False: 
            ...
            if df4.empty ==False: 
                ...
                df6 = self.get_equalvalue(df5, 'motivo_discrepancia', 'F5 NO RECIBIDO', 'MDI', 'Registro con motivo de disc: F5 no recibido')
                ...
                if df7.empty == False: 
                    df8 = self.get_diffqty_pro_f5(df7,  self.qty_column, 'cant_recibida', dqpcol, 'transfer', comment)
    ...
    def f5_verify_local(self, f5, status, yyyy, dqpcol, local):
        ...
        if df2.empty ==False: 
            ...
            if df4.empty ==False: 
                ...
                if df7.empty == False: 
                    df8 = self.get_diffqty_pro_f5(df7,  self.qty_column, 'cant_recibida', dqpcol, 'transfer', comment)
                    df9 = self.get_diffvalue(df8, 'local_recep', local, 'NCL', f'Registro con local diferente a {local}')
    ...
    def f5_verify_local_list(self, f5, status, yyyy, dqpcol, local , locales):
        ...
        if df2.empty ==False: 
            ...
            if df4.empty ==False: 
                ...
                df7 = self.get_diffvalue(df6, 'aaaa reserva', yyyy, 'NAA', f'Registro con año de reserva diferente a {yyyy}')
                comment = 'Cantidad de las NCs de un F5 != (cantidad pickeada | cantidad recibida del F5)'
                df8, lista = self.get_notinlist(df7, 'local_recep', locales, 'NCL', f'Registro con local diferente a {local}')
                if df8.empty == False: 
                    df9 = self.get_diffqty_pro_f5(df7,  self.qty_column, 'cant_recibida', dqpcol, 'transfer', comment)
                    iokf5 = df9[self.index_column].values
                    self.update_db(iokf5, 'GCO','OKK')
                    self.update_db(iokf5, 'Comentario GCO', 'Coincidencia exacta F5+UPC+QTY')
                return lista 
    ...
    def f3_verify(self, f3, status, yyyy, dqpcol):
        ...
        if df2.empty == False: 
            df3 = self.get_duplicates( df2,['cod_aut_nc', 'local_trx','upc', 'cantidad_trx_actual'], 'Cod Aut + Local + UPC + Qty')
            ne = self.get_notfound( df3, f3, [self.fcols[0],self.upc_column], ['nro_devolucion','upc'], 'nro_devolucion', 'F3|UPC|QTY')
            df4 = pd.merge(df3, f3, left_on=[self.fcols[0],self.upc_column], right_on=['nro_devolucion','upc'])
            if df4.empty ==False: 
                df5 = self.get_equalvalue(df4, 'descripcion6', 'Anulado', 'ANU', 'Registro anulado')
                comment = f'La cantidad sumada de los {dqpcol} de un f3 es mayor que la cantidad del f3'
                if df5.empty == False:
                    df6 = self.get_diffqty_pro(df5, self.qty_column, 'cantidad',dqpcol, 'nro_devolucion' ,comment)
