...
if num_f4_files > 1: 
    for i in range(num_f4_files): 
        f4_name  = f'input/{f4_input_name}_{i}.txt'
        f4_aux = pd.read_csv(f4_name, sep=';', dtype='object', error_bad_lines=False)
        list_f4.append(f4_aux)
    f4 = pd.concat(list_f4, axis=0)
else:
    f4_name = f'input/{f4_input_name}.txt'
    f4 = pd.read_csv(f4_name, sep=';', dtype='object', error_bad_lines=False)
