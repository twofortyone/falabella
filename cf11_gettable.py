...
cf11_cd = pd.read_csv('output/cierres_f11/cd/210723-1748-cf11_cd_20-output.csv', sep=';', dtype='object')
tab = pd.read_csv('input/tc2.csv', sep=';', dtype='object')
...
# Funciones 
def get_tcp(bd, args):
    return math.ceil(bd.loc[bd['status_nuevo'].isin(args),'total_costo_promedio'].sum()/1e6)

def escribir(pos1, pos2):
    return  '100% revisado contra SRX. ' + pos1.map(str) + ' de novedad equivalente a ' + pos2.map(str) + ' M.'

# Casuisticas CD 
f4_db = ['error en cierre de f11', 'error en creacion de f11','f4 de merma','politica cambio agil','cierre x f4 dado baja crate prestamos', 'f4 de mermaf4 2020 cierre f11 2021']
...
casus = [f4_db, f3_dp, recibido, f4_ct, f4_rev, rec_20, f5, recupero, anudup, pend,[],[]]
...
tab.iloc[:,2] = pd.to_numeric(pd.Series(lista_tiendas))
tab.iloc[:,3] = tab.iloc[:,1] + tab.iloc[:,2] # Total cd y tiendas
tab.iloc[:,4] = ((tab.iloc[:,3]/tab.iloc[13,3])*100).apply(lambda x: f'{math.ceil(x)}%' if x is not None else x) # Porcentaje 
...
tab.to_excel(f'output/tab1.xlsx', sheet_name='tab1',index=False) 
